...
imp="$skills/gku-implement/SKILL.md"
...
grep -q '^## Do not touch' "$skills/gku-plan/SKILL.md" || note 'plan: the template no longer writes a "Do not touch" list'
...
flat "$skills/gku-fix/SKILL.md" | grep -q 'Do not refactor nearby code' || note 'fix: drive-by refactors are back'
