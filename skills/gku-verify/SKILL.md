...
`gku-reference/security-checklist.md`; `/gku-review` reads for these, this step exercises them.
...
Every row's evidence is output from this run, quoted — `gku-reference/exec.md`'s rule on fresh
evidence, and the reason this skill exists rather than a reading of the diff.
...
branch, or `pr-<n>` when the run was aimed at a pull request, per `gku-reference/reports.md` in this
