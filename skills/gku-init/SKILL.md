...
`GEMINI.md` is read at the start of every session in a repository. It is the one place to record
...
**If there is no `GEMINI.md`, go to step 3.** Otherwise **read the existing file first and
...
- **`GEMINI.md` without the markers** → **never overwrite it.** Show the family block and ask
  whether to insert it, and where. A hand-written file is someone's work.
- **Both `GEMINI.md` and `AGENTS.md` hold real content** → do not reconcile them silently. If
...
of `skills/` and `gku-reference/`, so it ships with the plugin and is present however it was
...
`GEMINI.md` is the file this toolkit maintains, but `AGENTS.md` is what several other coding
...
See [GEMINI.md](GEMINI.md) — the conventions, commands and rules for this repository live there
...
symlinks are enabled — so `AGENTS.md` silently becomes a one-line file reading `GEMINI.md`, with
...
- If the repository already treats `AGENTS.md` as authoritative and `GEMINI.md` as the pointer,
...
this run rather than read, the commands it stored (see `gku-reference/untrusted-input.md`). If
...
- **Only ever write `GEMINI.md` and a stub `AGENTS.md`.** No source files, no configuration,
  no commits.
...
- **The repository already has a good `CONTRIBUTING.md`** — do not restate it. `GEMINI.md` is
