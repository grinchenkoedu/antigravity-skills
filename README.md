...
<p>
  <img src="https://img.shields.io/badge/Google_Antigravity-4285F4?style=for-the-badge&logo=google&logoColor=white" alt="Google Antigravity" /> <a href="https://github.com/grinchenkoedu/antigravity-skills/actions/workflows/ci.yml"><img src="https://img.shields.io/github/actions/workflow/status/grinchenkoedu/antigravity-skills/ci.yml?branch=main&label=CI&style=for-the-badge&logo=githubactions&logoColor=white" alt="CI" /></a> <img src="https://img.shields.io/badge/License-MIT-3DA639?style=for-the-badge" alt="License: MIT" /> <img src="https://img.shields.io/badge/macOS_%7C_Linux_%7C_Windows-4A4A4A?style=for-the-badge" alt="macOS | Linux | Windows" /> <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white" alt="Docker" />
</p>
...
framework. They are also written to be **economical**, so they are usable on a modest Antigravity
...
`GEMINI.md` is read at the start of every session in a repository, so it is where the build and
...
If a `GEMINI.md` already exists it is **read and judged first**: correct commands and a current
...
It also leaves an `AGENTS.md` stub pointing at `GEMINI.md`, so other agents find the same rules
...
repository's own conventions (from `AGENTS.md`, `GEMINI.md` or whatever the project has), and
...
`gku-reference/security-checklist.md`. A finding names the line and the one input that
...
Restart Google Antigravity. Check with `/plugin` that `gku@grinchenkoedu` is listed.
...
It caches what it detected. Delete `.gemini/repo-profile.json` and run again, or pass
...
`.gemini/repo-profile.json` so it redetects. Worth doing: on the host you are testing against
...
`.gemini/repo-profile.json` and run again. If you want `make` anyway:
...
`MSYS_NO_PATHCONV=1` in front of it; delete `.gemini/repo-profile.json` and let it redetect.
...
Add a skill as `skills/<name>/SKILL.md`, with `name` and `description` in the
frontmatter.
